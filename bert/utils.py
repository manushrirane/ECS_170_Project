...
def load_imdb(tokenizer_name='bert-base-uncased', max_length=256, split='train'):
...
# Placeholder hook to load MathQA/GSM8K - user will supply files or use repo
def load_mathqa_sample(path=None, tokenizer=None, max_len=256):
    # This is a placeholder: you should download/prepare MathQA/GSM8K and put them into plain text JSONL with 'question' and 'answer'.
    # Example: load from local jsonl into a small Dataset. Here we'll only create a dummy small math dataset for prototyping.
...
# nvidia-smi poller (runs in background) -> writes to CSV
def start_nvml_logger(outpath='gpu_log.csv', interval=1.0):
        ...
        with open(outpath, 'w') as f:
            ...
            while not stop.is_set():
                try:
                    ...
                    # handle multi-gpu by averaging
                    parts = [s.strip() for s in line.split(';') if s.strip()]
                    # take first GPU for now
                    if parts:
                        vals = parts[0].split(',')
    ...
    t.start()
    return stop  # call stop.set() to end
